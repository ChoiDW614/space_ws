...
echo "alias cb='cd \${DEMO_DIR} && colcon build --packages-select canadarm mars_rover simulation --parallel-workers 4 --cmake-args -DCMAKE_BUILD_TYPE=Release'" >> ~/.bashrc
...
cd ~/demos_ws && colcon build --packages-select canadarm mars_rover simulation --parallel-workers 4 --cmake-args -DCMAKE_BUILD_TYPE=Release
exec "$@"
